...
- **[`scaffold-mvp`](docs/pt-br/scaffold-mvp.md)**: Inicializa um novo repositório .NET/Blazor/Angular após o alinhamento de domínio/spec. Instala o harness de agente, propõe uma stack produtiva e gera o esqueleto inicial do projeto, AD-0001 e stubs.
- **[`create-issues`](docs/pt-br/create-issues.md)**: Converte gaps aprovados, roadmap e specs em GitHub Issues com fatias verticais e links de dependência. Utiliza a estrutura do template de SPEC SDD ao criar Issues a partir de especificações.
...
- **[`qa-analyst`](docs/pt-br/qa-analyst.md)**: Ciclo completo de QA — análise de requisitos, plano de testes, casos de teste, execução, relatórios de bugs e melhoria contínua de processos.
- **[`diagnose`](docs/pt-br/diagnose.md)**: Diagnóstico disciplinado e loop de re-validação para bugs difíceis e regressões de performance — loops de feedback apertados, hipóteses ranqueadas falsificáveis, playbooks por stack para .NET/Angular/Python/React, self-debug do agente e auditoria de workflows de IA.
- **[`improve-codebase-architecture`](docs/pt-br/improve-codebase-architecture.md)**: Identifica oportunidades de aprofundamento arquitetural lendo `.claude/CONTEXT.md`, `.claude/MEMORY.md` e `docs/architecture/`, e gera um relatório em HTML.
- **[`architecture`](docs/pt-br/architecture.md)**: A dona única de `docs/architecture/`. Roteia ADRs, docs de design e diagramas para `drawio-architecture`, `mermaid-architecture` e a skill opcional `archify` (HTML interativo, usada apenas quando já instalada — nunca instalada em tempo de execução), e encerra com a auditoria de evidências `gap-analysis` que ela possui.
...
|-----|-----------|-----------|
| Claude Code, OpenCode, Devin Desktop | `/spec-driven` e `/spec-driven:<skill>` | shims `commands/spec-driven:<skill>.md` |
| Devin CLI | `/<skill>` | skills são slash commands nativos — sem shims; instaladas em `~/.config/devin/skills/` |
...
Um workflow (`.github/workflows/lobehub-publish.yml`) publica todas as skills quando uma tag `v*` é criada, uma release é publicada ou quando executado manualmente (`workflow_dispatch`). Para habilitar, adicione dois secrets no repositório:
